"""
Label the manually curated POIs with the nature/culture/food/sport themes.

Reads ``data/out/selected_city_pois_2.json`` and writes a distilled file to
``data/out/selected_city_pois_2_llm_theme_labeled.json`` containing only the
requested fields plus the new theme labels and reasons.
"""
...
INPUT_PATH = Path("data/out/selected_city_pois_2.json")
...
- culture: museums, castles, churches, historic architecture, guided city
           ...
           parliaments. Spa and wellness experiences belong here.
...
Exactly one theme must be marked true—the single best fit for the experience.
Set the other three themes to false and provide a concise reason for each
decision. When a theme is false, explain in one short sentence why it was
rejected in favour of the primary theme. If an attraction does not perfectly
match any theme, choose the closest fit rather than leaving multiple true
values or all false.
...
def read_pois(path: Path) -> List[Dict]:
    return json.loads(path.read_text(encoding="utf-8"))


def classify_theme(client: OpenAI, poi: Dict) -> Dict[str, object]:
    context = json.dumps(
        {
            "name": poi.get("name"),
            "city": poi.get("city"),
...
"""
        ...
        if sum(bool(payload.get(key)) for key in ("nature", "culture", "food", "sport")) == 1:
            break
        reminder = "\n\n⚠️ Exactly ONE theme may be true. Re-evaluate and answer again."

    if payload is None:
        payload = {}
    return enforce_single_theme(poi, payload)
...
def heuristic_theme_selection(text: str, candidates: List[str]) -> str:
    ordered_candidates = [theme for theme in candidates if theme in THEME_ORDER]
...
def trim_payload(poi: Dict, labels: Dict) -> Dict:
    result = {key: poi.get(key) for key in REQUIRED_KEYS if key not in labels}
    result.update({key: labels.get(key) for key in REQUIRED_KEYS if key in labels})
    return result


def main() -> None:
    client = load_client()
    pois = read_pois(INPUT_PATH)
    relabeled = []

    for idx, poi in enumerate(pois, start=1):
        labels = classify_theme(client, poi)
        relabeled.append(trim_payload(poi, labels))
        print(f"[{idx}/{len(pois)}] {poi.get('name', 'Unknown')} -> {next(k for k, v in labels.items() if k in ('nature','culture','food','sport') and v)}")

    OUTPUT_PATH.write_text(json.dumps(relabeled, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote {len(relabeled)} POIs to {OUTPUT_PATH}")
